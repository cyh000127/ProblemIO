          ...
          <div class="flex flex-col gap-6">
            ...
            <div class="flex flex-col gap-4">
              ...
              <div class="flex flex-col gap-2">
                ...
                <InputText v-model="quizForm.title" placeholder="Enter quiz title" class="w-full" />
              </div>

              <div class="flex flex-col gap-2">
                <label class="text-sm font-medium">Description</label>
                <Textarea v-model="quizForm.description" placeholder="Describe your quiz" rows="3" class="w-full" />
              </div>
              ...
              <div class="flex flex-col gap-2">
                ...
                <div class="flex items-center gap-3 flex-wrap">
                  ...
                  />
                  <Button v-if="quizForm.thumbnailUrl" label="Remove" severity="secondary" text icon="pi pi-times" @click="clearThumbnail" />
                </div>
                <InputText v-model="quizForm.thumbnailUrl" placeholder="https://example.com/image.jpg" class="w-full" />
            ...
            <div class="flex flex-col gap-4">
              ...
              <Card v-for="(question, index) in quizForm.questions" :key="index" class="question-card">
                <template #header>
                  <div class="p-3 border-bottom-1 surface-border flex justify-between items-center">
                    <span class="font-semibold">Question {{ question.questionOrder }}</span>
                    <Button icon="pi pi-trash" severity="danger" text rounded @click="removeQuestion(index)" />
                  </div>
                </template>
                <template #content>
                  <div class="flex flex-col gap-4">
                    <div class="flex flex-col gap-2">
                      ...
                      <Textarea v-model="question.description" placeholder="Enter question description" rows="2" class="w-full" />
                    </div>
                    ...
                    <div class="flex flex-col gap-2">
                      ...
                      <div class="flex items-center gap-3 flex-wrap">
                        ...
                        />
                        <Button v-if="question.imageUrl" label="Remove" severity="secondary" text icon="pi pi-times" @click="clearQuestionImage(index)" />
                      </div>
                      <InputText v-model="question.imageUrl" placeholder="https://example.com/question-image.jpg" class="w-full" />
                    ...
                    <div class="flex flex-col gap-2">
                      ...
                      <small class="text-color-secondary mb-2">Enter multiple answers (synonyms, variations). Press Enter to add each answer.</small>
                      <Chips v-model="question.answers" placeholder="Type answer and press Enter" class="w-full" />
                    </div>
                  </div>
                </template>
              </Card>
            ...
            <div class="flex justify-end gap-3">
              ...
              <Button label="Complete Creation" icon="pi pi-check" :loading="submitting" :disabled="!isFormValid || thumbnailUploading || questionUploadingIndex !== null" @click="handleSubmit" />
            </div>
          </div>
        </template>
      </Card>
